...
# SD1: Epigenome-wide study buccal samples
.csv file
...
# SD2: Epigenome-wide study cervical samples
...
# SD3: Epigenome-wide study cervical samples
.csv file
...
blood <- readRDS(here("1-ewas", "3-blood", "delta-beta.Rds")) |> 
  ...
  dplyr::mutate(padj = p.adjust(type, method = 'BH')) |> 
  tibble::rownames_to_column('cg') |> 
  dplyr::rename(db_neutrophil = db_immune,
                db_other = db_epithelial)
...
# Extract info delta-beta
tmp <- man.epic |>
  dplyr::select(Name, chr, pos, UCSC_RefGene_Name, UCSC_RefGene_Group, Relation_to_Island)
...
# Select relevant columns for supplement
result <- result |>
  dplyr::select(cg, chr, pos, dir_type, db_neutrophil, db_other, pvalue, padj, UCSC_RefGene_Name, UCSC_RefGene_Group, Relation_to_Island)

# Save to .csv
write.csv(result, file.path(supPath, "ST3.csv"), row.names=FALSE)
```

# SD4: Overlapping genomic regions, buccal and cervical
