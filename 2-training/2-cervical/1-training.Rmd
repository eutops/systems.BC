---
title: "Training on cervical samples"
...
library(here)
here::i_am("2-training/1-buccal/1-training.Rmd")
...
pdat <- rbind(ridge_epi, lasso_epi, ridge_imm, lasso_imm, ridge_combined, lasso_combined, ridge_p, lasso_p)
...
pdat$Type <- factor(pdat$Type, levels = c("Ridge - Combined", "Lasso - Combined", "Ridge - Epithelial", "Lasso - Epithelial", "Ridge - Immune", "Lasso - Immune", "Ridge - pval", "Lasso - pval"))

plotly::ggplotly(plot_performance_compare(pdat, pdat$AUC_val, sets = 8))

# save for plotting
save(pdat, file=here("0-data","dataframes-plotting","cervical-training.Rdata"))

pdat <- pdat[order(pdat$AUC_val,decreasing = TRUE),]
head(pdat)
```

Max: n = 15000 for Ridge pval Ridge (AUC_val = `r signif(pdat[which.max(pdat$AUC_val),]$AUC_val)`) 

### Out-of-bag estimate AUC

```{r fig.align = "center", fig.width = 10, fig.height = 4.5}
plotly::ggplotly(plot_performance_compare(pdat, pdat$AUC_oob, sets = 8))

pdat <- pdat[order(pdat$AUC_oob,decreasing = TRUE),]
head(pdat)
```

* max oob at n=30000 for Ridge pval (AUC_oob = `r signif(pdat[which.max(pdat$AUC_oob),]$AUC_oob)`) 
...
* Lasso/Ridge have similar AUC in int val, slight difference in oob
* Slope is better for lasso, but lasso is less 'stable'
