...
sigCpGs <- subset(blood, type < 0.05, select = cg)
sigCpGs <- dplyr::pull(sigCpGs,cg)

# Write DMPs to .txt file for eFORGE upload - max 1000 probes
sigCpGs <- blood[blood$type < 0.05, ]
sigCpGs <- sigCpGs[order(sigCpGs$type), ]
sigCpGs <- head(sigCpGs, 1000)
sigCpGs <- dplyr::pull(sigCpGs,cg)
writeLines(sigCpGs, here("1-ewas", "3-blood", "dmps.txt"))
...
On CpGs significant before correction (unadjusted p-value)
...
save(gst.go, gst.kegg,file=here("1-ewas","3-blood","2-output","gsea-missMethyl.Rdata"))

load(here("1-ewas","3-blood","2-output","gsea-missMethyl.Rdata"))
topGSA(gst.go)
topGSA(gst.kegg)
```
...
## save results -------------------
save(bp, mf, cc, epi_hypo_react_all, file=here("1-ewas","3-blood","2-output","gsea-clusterProfiler.Rdata"))
