...
here::i_am("1-ewas/3-blood/1-ewas-analysis.Rmd")
...
An EWAS is run on the entire discovery set of blood samples. Here, we visualise covariates and prepare a script that can be run in terminal as this is quicker.
...
* Adjusting for the proportions of neutrophils (biggest cell proportion)
...
pheno <- droplevels(data[data$set==setname & data$sampletype==sampletypename,]) |> 
  ...
  # code 'Neutro' as ic
  dplyr::mutate(ic = hepidish_Neutro)
save(pheno, file = here("1-ewas", "3-blood", "pheno.Rdata"))

# Set paths
pheno <- here("1-ewas","3-blood", "pheno.Rdata")
beta <- here(db_path, "data", "3c-blood", "beta_merged.Rdata")
output <- here("1-ewas", "3-blood")
...
# Save script
write(out, here("1-ewas", "3-blood", "1-run-db.R"))
...
Note: db epi is now Neutro, db imm high neutrophil

## Significant sites

* `r sum(db$type<0.05)` significant CpGs
* `r sum(p.adjust(db$type, method = 'BH') < 0.05)` after BH adjustment
* maximum p value (adj): `r format(min(db$type[db$padj >= 0.05]), scientific = T)`